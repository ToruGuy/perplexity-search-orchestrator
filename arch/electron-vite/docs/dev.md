...
### Limitations of Sandboxing [​](#limitations-of-sandboxing)
...
## Multiple Windows App [​](#multiple-windows-app)
...
Check out the [Using HMR](./hmr) section for more details.

## Passing Arguments to Electron App [​](#passing-arguments-to-electron-app)
...
electron-vite already supports `--inspect`, `--inspect-brk`, `--remote-debugging-port` and `--no-sandbox` commands, so you don't need to do this for these commands. See [Command Line Interface](./cli#dev-options) for more details.
...
## Utility Process and Child Process [​](#utility-process-and-child-process)
...
### ESM and CJS Syntax Compatibility [​](#esm-and-cjs-syntax-compatibility)

electron-vite has made some compatibility with ES Modules and CommonJS syntax, allowing developers to freely switch between the two syntaxes with minimal migration work. But it should be noted that [Source Code Protection](./source-code-protection) currently only supports CommonJS.


---

Source: [https://electron-vite.org/guide/dev](https://electron-vite.org/guide/dev)