...
See [Vite's troubleshooting guide](https://vitejs.dev/guide/troubleshooting) and [Rollup's troubleshooting guide](https://rollupjs.org/troubleshooting/) for more information too.
...
### `The CJS build of Vite's Node API is deprecated` [​](#the-cjs-build-of-vite-s-node-api-is-deprecated)
...
Note that when adding `"type": "module"` in the project `package.json`, if Electron support ESM (Electron 28+), it will be built as ESM and you need to read the [ESM Support in Electron](./dev#esm-support-in-electron) guide first. But if ESM is not supported, it will be built as CommonJS and all files will have a `.cjs` extension.
...
### `Unable to load preload scripts -> Error: module not found: 'XXX'` [​](#unable-to-load-preload-scripts-error-module-not-found-xxx)
...
### `Uncaught Error: Module "XXX" has been externalized for browser compatibility.` or `Uncaught ReferenceError: __dirname is not defined` [​](#uncaught-error-module-xxx-has-been-externalized-for-browser-compatibility-or-uncaught-referenceerror-dirname-is-not-defined)
...
It is recommended to use `preload scripts` for development and avoiding Node.js modules for renderer code. if you want to do this，you can add polyfills manually， see [nodeIntegration](/guide/dev#nodeintegration) for more details.
  ...
  * Related: [Module externalized for browser compatibility](https://vitejs.dev/guide/troubleshooting#module-externalized-for-browser-compatibility)


## Build [​](#build)

### `Error [ERR_REQUIRE_ESM]: require() of ES Module` [​](#error-err-require-esm-require-of-es-module)
...
### `vue-router` or `react-router-dom` works in development but not production [​](#vue-router-or-react-router-dom-works-in-development-but-not-production)
...
### `A JavaScript error occurred in the main process -> Error: Cannot find module 'XXX'` [​](#a-javascript-error-occurred-in-the-main-process-error-cannot-find-module-xxx)
...
### `A JavaScript error occurred in the main process -> Error: Invaild or incompatible cached data (cachedDataRejected)` [​](#a-javascript-error-occurred-in-the-main-process-error-invaild-or-incompatible-cached-data-cacheddatarejected)
...
Source: [https://electron-vite.org/guide/troubleshooting](https://electron-vite.org/guide/troubleshooting)