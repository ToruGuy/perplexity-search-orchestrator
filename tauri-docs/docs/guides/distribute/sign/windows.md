...
Code signing is required on Windows to allow your application to be listed in the [Microsoft Store](https://apps.microsoft.com/) and to prevent a [SmartScreen](https://en.wikipedia.org/wiki/Microsoft_SmartScreen) warning that your application is not trusted and can not be started, when downloaded from the browser.

It is not required to execute your application on Windows, as long as your end user is okay with ignoring the [SmartScreen](https://en.wikipedia.org/wiki/Microsoft_SmartScreen) warning or your user does not download via the browser. This guide covers signing via OV (Organization Validated) certificates and Azure Key Vault. If you use any other signing mechanism not documented here, such as EV (Extended Validation) certificates, check out your certificate issuer documentation and refer to the [custom sign command](#custom-sign-command) section.

## [OV Certificates](#ov-certificates)

[Section titled “OV Certificates”](#ov-certificates)
...
There are a few things we have to do to get Windows prepared for code signing. This includes converting our certificate to a specific format, installing this certificate, and decoding the required information from the certificate.
  ...
  1. #### [Convert your `.cer` to `.pfx`](#convert-your-cer-to-pfx)

[Section titled “Convert your .cer to .pfx”](#convert-your-cer-to-pfx)
  ...
  2. #### [Import your `.pfx` file into the keystore.](#import-your-pfx-file-into-the-keystore)

[Section titled “Import your .pfx file into the keystore.”](#import-your-pfx-file-into-the-keystore)
  ...
  3. #### [Prepare Variables](#prepare-variables)

[Section titled “Prepare Variables”](#prepare-variables)
...
### [Prepare `tauri.conf.json` file](#prepare-tauriconfjson-file)

[Section titled “Prepare tauri.conf.json file”](#prepare-tauriconfjson-file)
...
### [Sign your application with GitHub Actions.](#sign-your-application-with-github-actions)

[Section titled “Sign your application with GitHub Actions.”](#sign-your-application-with-github-actions)

We can also create a workflow to sign the application with GitHub actions.

#### [GitHub Secrets](#github-secrets)

[Section titled “GitHub Secrets”](#github-secrets)
...
[Section titled “Workflow Modifications”](#workflow-modifications)
...
In the [Azure Portal](https://portal.azure.com) navigate to the [Key vaults service](https://portal.azure.com/#browse/Microsoft.KeyVault%2Fvaults) to create a new key vault by clicking the “Create” button. Remember the “Key vault name” as you will need that information to configure the certificate URL.
...
[relic](https://github.com/sassoftware/relic) uses a configuration file to determine which signing key it should use. For Azure Key Vault you also need the certificate URL. Create a `relic.conf` file in the `src-tauri` folder and configure relic to use your certificate:
...
To configure Tauri to use your Azure Key Vault configuration for signing change the [bundle > windows > signCommand](/reference/config/#signcommand) config value:
...
[relic](https://github.com/sassoftware/relic) must authenticate with Azure in order to load the certificate. In the Azure portal landing page, go to the “Microsoft Entra ID” service and head to the “Manage > App registrations” page. Click “New registration” to create a new app. After creating the app, you are redirected to the application details page where you can see the “Application (client) ID” and “Directory (tenant) ID” values. Set these IDs to the `AZURE_CLIENT_ID` and `AZURE_TENANT_ID` environment variables respectively.
...
## [Custom Sign Command](#custom-sign-command)

[Section titled “Custom Sign Command”](#custom-sign-command)

In the [Azure Key Vault](#azure-key-vault) documentation above we used a powerful Tauri Windows signing configuration to force the Tauri CLI to use a special shell command to sign Windows installer executables. The [bundle > windows > signCommand](/reference/config/#signcommand) configuration option can be used to use any codesign tool that can sign Windows executables.

## [Azure Code Signing](#azure-code-signing)

[Section titled “Azure Code Signing”](#azure-code-signing)
...
If you want to sign with Github Actions everything should be installed.
  ...
  2. [.NET](https://dotnet.microsoft.com/en-us/download/dotnet/8.0) (.NET 6 or later recommended)
  3. [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli-windows?tabs=azure-cli#install-or-update)
  4. [Signtool](https://learn.microsoft.com/en-us/dotnet/framework/tools/signtool-exe) (Windows 11 SDK 10.0.22000.0 or later recommended)
...
You need to install [trusted-signing-cli](https://github.com/Levminer/trusted-signing-cli) and configure your environment variables.
  ...
  1. #### [Install trusted-signing-cli](#install-trusted-signing-cli)

[Section titled “Install trusted-signing-cli”](#install-trusted-signing-cli)
...
[Section titled “Configure environment variables”](#configure-environment-variables)
  ...
  3. ### [Modify your `tauri.conf.json` file](#modify-your-tauriconfjson-file)

[Section titled “Modify your tauri.conf.json file”](#modify-your-tauriconfjson-file)
...
**Source:** [https://v2.tauri.app/distribute/sign/windows](https://v2.tauri.app/distribute/sign/windows)